...
TPU_TYPE=$(gcloud container node-pools describe $NODEPOOL --region=$GKE_REGION --cluster=$GKE_CLUSTER --project=$GKE_PROJECT | grep "machineType" | awk '{print $2}')
if [ -z "$TPU_TYPE" ]; then exit; fi;
...
YAML_VARS='$JOB_NAME $DOCKER_IMAGE $NODEPOOL $NUM_SUBSLICES $TPU_TOPOLOGY $TPU_ACCELERATOR $TPU_TYPE $COMPLETIONS $MEMORY_PER_HOST $TPU_CHIPS_PER_HOST $GKE_PROJECT $GKE_REGION $GKE_CLUSTER $LIBTPU_ARGS'
...
STATUS=$(kubectl apply -f find_badhosts.yaml)
echo $STATUS
